...
A CNN apresenta alta popularidade para lidar com problemas de reconhecimento de imagem, trazendo um nível de precisão e escalabilidade. Por conta disso, existem diversas arquiteturas de CNN, lindando o problema com diferentes abordagens. Dentro da vasta gama de opções das arquiteturas de CNN, é possível encontrar arquiteturas como: \textit{LeNet}, \textit{AlexNet}, \textit{ZFNet}, \textit{VGGNet}, \textit{ResNet}, \textit{GoogleLeNet}, entre outras. 

Uma Rede Neural Residual (ResNet, do inglês \textit{Residual Network}) é uma arquitetura de CNN que permite melhorar a eficiência do treinamento e resolver problemas de degradação de desempenho. A ResNet é capaz de se deslocar por várias camadas da rede, graças à utilização de atalhos. Além disso, a ResNet utiliza blocos residuais, permitindo que a informação seja passada diretamente para camadas posteriores. 
...
Listada na Subseção \ref{redesresiduais}, a arquitetura ResNet se destaca por ser focada em reconhecimento de imagem, além de ter ganho o Desafio de Reconhecimento Visual em Larga Escala ImageNet (ILSVRC, do inglês \textit{ImageNet Large Scale Visual Recognition Challenge}) em 2015, atingindo uma taxa de erro top 5 de 3,6\% com uma CNN composta por 152 camadas (ResNet152).

Entretanto, por mais que a ResNet152 seja o modelo mais profundo, e que captura os recursos mais abstratos e complexos, é também o modelo mais pesado e é computacionalmente caro para treinar e implantar em dispositivos com recursos limitados. Existem diferentes variantes que rodam no mesmo conceito, mas com diferentes números de camadas, assim como ResNet18, ResNet34, ResNet50 e ResNet101. 

Dentre essas, a ResNet50 se ressalta por ter um equilíbrio na profundidade de camadas além de que, de acordo com \citeonline{resnet50analisys}, as ResNet's com 50, 101 e 152 camadas são mais precisas do que as ResNet's de 18 e 34 camadas. Ao reduzir as opções para 50, 101 e 152 camadas, foi possível definir a ResNet50 como a escolhida, uma vez que as arquiteturas com 101 e 152 camadas requerem mais recursos computacional do que a de 50.
...
% Inicialmente, para o desenvolvimento do código, foi feita a importação das bibliotecas necessárias, conforme descrito na Subseção \ref{techs}, para o desenvolvimento do código. Essas fornecem funcionalidades e recursos necessários para a implementação do algoritmo. 
...
Em seguida, o pré-processamento das imagens foi realizado, gerando assim o vetor para o treinamento. Dentro do método que transforma os dados em vetor, existe um parâmetro chamado \textit{rescale} utilizado para fazer a normalização dos valores dos \textit{pixels} das imagens, transformando os valores originais dos \textit{pixels} em uma escala específica. Geralmente, essa escala é utilizada com o valor de 1/255, uma vez que as imagens utilizadas na implementação consistem em coeficientes Vermelho, Verde, Azul (RGB, do inglês \textit{Red, Green, Blue}), ou seja, cada \textit{pixel} tem um valor entre 0 e 255. Entretanto, esses valores são grandes demais para o modelo proposto processar, conforme a taxa de aprendizado comum. Por conta disso, os valores foram realinhados para se distinguir entre 0 e 1.

Além disso, foi utilizado um método para pré-processar os dados carregados para o treinamento. Nesse método, foi especificado o diretório onde os dados estão localizados, o tamanho do redimensionamento das imagens para 224x224 (uma vez que a rede neural utilizada requer esses valores \cite{kerasresnet50}), o modo de classificação como binário (onde 0 classifica pacientes sem câncer e 1 classifica pacientes com câncer), e o tamanho do \textit{batch}, ou seja, o número de imagens carregadas por vez durante o treinamento.
...
Na implementação do modelo, foi utilizada a arquitetura \textit{ResNet50} para realizar o \textit{Transfer Leaning}, modelo de rede neural que utiliza blocos residuais, permitindo que a informação seja passada diretamente para camadas posteriores. Com essa informação, foi carregado o pré-processamento da \textit{ResNet} para que as imagens possam ser entendíveis para a rede utilizada. Em seguida, foi utilizado um método para pré-processar os dados que foram carregados para o treinamento e para a validação.


%%%%%%%%%%%%%%%%%%%%%%%     FUNÇÕES DE ATIVAÇÃO       %%%%%%%%%%%%%%%%%%%%%%

\subsubsection{\esp Camadas e Funções de Ativação do Modelo} \label{camadas}
Com as imagens de treinamento e validação pré-processadas, foi feita a importação da rede neural \textit{ResNet} que aplica o método de \textit{Transfer Learning}. O modelo foi carregado, configurando parâmetros que indicam a não inclusão da camada superior do modelo (ou seja, foram usadas somente camadas convolucionais), e especificando o formato esperado das imagens de entrada para o modelo (já definidas anteriormente pelo método que pré-processa os dados, citado na Subseção \ref{transfer}).

Um modelo ideal recebe uma lista de camadas onde cada camada tem uma função específica. A Figura \ref{fig:cnn} apresenta o diagrama das camadas \textit{Conv\_1}, \textit{Conv\_2} e \textit{Max-Pooling}, responsáveis por extrair e detectar padrões das imagens, e a camada \textit{Fully-Connected}, que permite que o modelo aprenda padrões mais sofisticados. No modelo proposto, a \textit{ResNet} disponibiliza os dados extraídos, mas não implementa a camada \textit{Fully-Connected}.
...
Para declarar o modelo da implementação, foi necessário ter uma lista de camadas. Para isso, foi feita a importação de uma Interface de Programação de Aplicações (API, do inglês \textit{Application Programming Interface}) chamada \textit{Sequential}, que permite adicionar camadas sequencialmente ao modelo. Além disso, foi importada também a camada \textit{Dense} que representa uma camada totalmente conectada, utilizada para mapear as entradas para as saídas. Juntamente com as camadas já citadas, a camada \textit{Flatten} foi usada para achatar os dados de entrada em um vetor unidimensional. Por fim, a camada \textit{GlobalAveragePooling2D} realiza a operação de \textit{pooling} médio, utilizada para reduzir a dimensionalidade dos recursos extraídos das camadas convolucionais anteriores. Para gerar arquivos de \textit{log} úteis para avaliar o desempenho do modelo, foi utilizado o \textit{TensorBoard}.

A \textit{ResNet}, por mais que completa, não consegue gerar as camadas \textit{Fully Connected}. Para contornar isso, foi feito um procedimento onde todas as camadas do modelo são percorridas, e a propriedade \textit{trainable} foi definida como Falso para cada uma delas. Isso significa que as camadas do modelo base não foram treinadas durante o processo de treinamento subsequente. Ao definir essa propriedade como Falso, as camadas do modelo base permaneceram com os pesos pré-treinados e não foram atualizadas durante o treinamento adicional. 

Em seguida, foi definido um modelo de rede neural passando uma lista de camadas, com a camada base definida como a camada pré-treinada. As outras camadas passadas (1 camada \textit{GlobalAveragePooling2D} e 4 camadas \textit{Dense}) realizam operações para reduzir a dimensionalidade dos recursos extraídos de camadas anteriores. A camada \textit{Dense} realiza transformações lineares nos dados de entrada, e cada chamada contém parâmetros que se distinguem, assim como a quantidade de unidades (2, 36, 128 ou 1024) e a função do modo de ativação ('\textit{relu}', ou '\textit{softmax}'). 
...
%         \includegraphics[width=\textwidth]{figuras/model10pred1.png}
%         \label{fig:predicao1}
...
%         \includegraphics[width=\textwidth]{figuras/model10pred2.png}
%         \label{fig:predicao2}
%         \textbf{\footnotesize Fonte: \href{http://visual.ic.uff.br/dmi}{\textit{Visual Lab DMR}}}
%     \end{minipage}
% \end{figure}
...
Nesta seção, visa-se apresentar as ações à serem tomadas a partir do segundo semestre de 2023, explícito na Figura \ref{fig:cronograma}.

\begin{figure}[ht]
 	\centering	
 	\caption[\hspace{0.1cm}Grade Computacional.]{Cronograma}
 	\vspace{-0.3cm}
 	\includegraphics[width=1\textwidth]{figuras/cronograma.PNG}
 	\captionsetup{justification=centering}
	\vspace{-0.5cm}
  \\\textbf{\footnotesize Fonte: Elaborado pelos autores}
