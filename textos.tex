...
base de dados (selecionar imagens)
->
pré-processamento (filtros e normalização etc)
->
...
validação de resultados
    ...
    pegar base de dados, normalizar, processar, jogar num classificador (rede neural covulacional) e retorna a classificação
...
\subsection{\esp Materiais}
As amostras de imagens termográficas foram obtidas através do trabalho realizado para o projeto PROENG \citeonline{database}, realizado com o objetivo de auxiliar no diagnóstico por imagens médicas, desde a aquisição das imagens e sua organização no banco de dados, até suas análises e técnicas de Aprendizado de Máquina. O Banco de Dados para Pesquisas em Mastologia (DMR, do inglês \textit{Database for Mastology Research}) \href{http://visual.ic.uff.br/dmi}{Visual Lab DMR} é uma plataforma online que mantém imagens termográficas e mamografias, na qual pessoas voluntarias podem inserir suas imagens para ajudar. %melhorar o final

quantas imagens foram usadas
...
pip install imutils
python -m pip install PySimpleGUI imageCut - > def para corte da imagem (a imagem recortada é salva como cropped.jpg na pasta photos do projeto)
...
Falar sobre as bibliotecas, linguagem, algoritmo, configuração da rede neural, versão
...
Yago: Estamos utilizando 4 bibliotecas, 
1. tensorflow -> biblioteca que tem métodos para criar rede neural
2. keras -> escrever de um jeito melhor em vez de ter que implementar o tensorflow
3. numpy que é pra transformar os dados que a gnt tem em vetores (fotos em vetores de 0 ou 1 em escala de cinza)
4. matplotlib que é pra plotar gráficos de métricas etc
5. escarlene para usar matrizes


cria uma rede e treinando c uma rede neural -> 
usando resnet

pondo a imagem lá, fazendo modelo no nosso usando a resnet rede resnet 50 p poder gerar classificador

ai usando 30 épocas e 45 partes por épocas, peso 34 p poder fazer os valores da rede neural

as funções usando de otimizaçao
1. adol -> faz cálculos não linear e pega medida de gradiente estocastico baseado na estimativa adaptativa de momentos de 1a e 2a ordem

pega uma reta diagonal, pega pontinhos próximos, faz cálculos -> peso

treinando, fazendo rede neural com 5 neuronios finais
10 camadas dif, layers
trocando as camadas, trocando filtros p ver se tem resultado bacana


temq explicar qq a fun~cáo de ativacao faz, como q ta treinando em cada camada, e se temq falar sobre o tensorflow e sqlearn?

\subsection{\esp Cronograma}
8 tarefas e datas
